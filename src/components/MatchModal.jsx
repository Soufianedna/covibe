      ...
      <div className="relative max-w-2xl w-full">
        ...
        <div className="flex items-center justify-center gap-8 mb-8 relative">
          <div className="relative">
            {currentUser.photo_url ? (
              <img src={currentUser.photo_url} alt={currentUser.name} className="w-40 h-40 md:w-48 md:h-48 rounded-full object-cover border-8 border-violet-500 shadow-2xl shadow-violet-500/50" />
            ) : (
              <div className="w-40 h-40 md:w-48 md:h-48 rounded-full bg-slate-700 flex items-center justify-center text-6xl border-8 border-violet-500">👤</div>
            )}
          </div>
        ...
        <div className="bg-slate-800/50 backdrop-blur-lg border border-violet-500/30 rounded-3xl p-8">
          <p className="text-white text-center mb-6 text-lg">
            <span className="font-bold text-violet-400">{currentUser.name}</span>{ t('and') }<span className="font-bold text-purple-400">{matchedUser.name}</span>, { t('canChatSafely') }
          </p>

          <div className="space-y-4">
            <button onClick={onOpenChat} className="flex items-center justify-center gap-3 w-full py-4 bg-gradient-to-r from-violet-600 to-indigo-500 text-white rounded-xl font-bold text-lg hover:shadow-2xl hover:shadow-violet-500/50 transition-all">
              <MessageCircle size={24} />
              { t('sendMessageTo') } {matchedUser.name.split(' ')[0]}
            </button>
