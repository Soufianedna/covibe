...
// Texte CoVibe
const textGradient = ctx.createLinearGradient(size*0.3, 0, size*0.7, 0);
textGradient.addColorStop(0, '#EC4899');
textGradient.addColorStop(0.5, '#A855F7');
textGradient.addColorStop(1, '#06B6D4');

ctx.fillStyle = textGradient;
ctx.font = `bold ${size * 0.1}px -apple-system, sans-serif`;
ctx.textAlign = 'center';
ctx.textBaseline = 'middle';
ctx.fillText('CoVibe', size / 2, size / 2 + size * 0.22);

writeFileSync('/Users/souf/Downloads/files/splash.png', canvas.toBuffer('image/png'));
console.log('✅ Splash avec CoVibe généré');
